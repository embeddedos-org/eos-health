#!/usr/bin/env python3
"""
EoS Health — Master Verification Runner
Runs all L1 verification checks and produces a consolidated report.
"""
...
REPORT_PATH = VERIFICATION_DIR.parent / "VERIFICATION_REPORT.md"


def run_check(script: str, name: str) -> tuple[int, str]:
    """Run a verification script and return (exit_code, output)."""
    start = time.time()
    result = subprocess.run(
        [sys.executable, str(VERIFICATION_DIR / script)],
        capture_output=True, text=True,
        cwd=str(VERIFICATION_DIR.parent)
    )
...
def extract_summary(output: str) -> str:
    """Extract the RESULTS line from checker output."""
    for line in output.splitlines():
        if 'RESULTS:' in line or 'TOTAL:' in line or 'STATUS:' in line:
            return line.strip()
...
def main():
    print("=" * 60)
    print("EoS Health — L1 Design Verification Suite")
    print(f"Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 60)

    checks = [
        ("check_firmware_syntax.py", "Firmware Syntax & Structure"),
        ("test_algorithms.py",       "Health Algorithm Unit Tests"),
        ("static_analysis.py",       "Static Code Analysis"),
        ("validate_hardware.py",     "Hardware & BOM Validation"),
    ]
    ...
    for script, name in checks:
        code, output, elapsed = run_check(script, name)
        summary = extract_summary(output)
        status = "✅ PASS" if code == 0 else "❌ FAIL"
        results.append({
            "name": name, "script": script, "code": code,
            "summary": summary, "elapsed": elapsed, "output": output
        })
        if code != 0:
            total_pass = False
    ...
    report = f"""# EoS Health — L1 Design Verification Report
...
"""
    ...
    overall = "✅ ALL L1 CHECKS PASSED" if total_pass else "⚠️ SOME CHECKS HAVE WARNINGS"
    report += f"""
...
"""
    ...
    print(f"\n{'='*60}")
    print(f"Verification report written to: VERIFICATION_REPORT.md")
    print(f"{'='*60}")
    print(f"\nOverall: {overall}")
